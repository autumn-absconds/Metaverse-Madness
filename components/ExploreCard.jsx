...
const ExploreCard = ({ id, imgUrl, title, active, handleClick, index }) => (
  ...
  >
    ...
    {active !== id ? (
      <h3 className='font-semibold sm:text-[26px] text-[18px] text-white absolute lg:bottom-20 z-0 lg:rotate-[-90deg] lg:origin-[0,0]'>
        {title}
      </h3>
    ) :
      <div className='absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]'>
        ...
        <p className='font-normal text-[16px] leading-[20px] text-white uppercase'>Entr the world</p>
        <h2 className='mt-[24px] font-semibold sm:text-[32] text-[24px] text-white'>
          {title}
        </h2>
